...
template <typename T>
CircularList<T>::const_iterator::const_iterator(const Node* firstNode,
                                                bool beginIterator) {
...
template <typename T>
const typename CircularList<T>::Node*
CircularList<T>::const_iterator::current_node() const {
...
template <typename T>
CircularList<T>::iterator::iterator(Node* firstNode, bool beginIterator) {
  myFirstNode = firstNode;
  myCurrentNode = firstNode;
  myFirstInitialized = beginIterator;
}
...
const typename CircularList<T>::Node* CircularList<T>::Offset(
    const Node* startNode, int offset) {
  if (!startNode) return nullptr;

  if (offset == 0) return startNode;

  const Node* node = startNode;
  if (offset > 0) {
    for (int i = 0; i < offset; ++i) node = node->next;
  } else {
    for (int i = 0; i > offset; --i) node = node->prev;
  }
  return node;
}
