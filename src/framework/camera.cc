...
namespace {
float distanceBetweenPointAndCube(const glm::vec3& pt,
                                  const glm::vec3& cubeMinPt,
                                  const glm::vec3& cubeMaxPt) {
  glm::vec3 closestPoint(std::max(cubeMinPt.x, std::min(pt.x, cubeMaxPt.x)),
...
void Camera::translate(const glm::vec2& screenOffset) {
  ...
  glm::vec3 offset =
      correctedOffset.x * xScreenAxis + correctedOffset.y * yScreenAxis;
...
void Camera::rotateAroundPivot(const glm::vec3& pivotPoint,
