...
uint32_t getHostVisibleMemoryIndex(VkPhysicalDevice physicalDevice) {
  ...
  for (uint32_t i = 0; i < memProps.memoryTypeCount; ++i) {
    ...
    VkFlags hostVisibleAndCoherent = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    if ((mt.propertyFlags & hostVisibleAndCoherent) != hostVisibleAndCoherent)
      continue;
    if (!hostVisibleMemIndexSet ||
        mt.propertyFlags & VK_MEMORY_PROPERTY_HOST_CACHED_BIT) {
...
GpuMemory::GpuMemory(VkPhysicalDevice physicalDevice, VkDevice device,
                     VkBufferUsageFlags usage, size_t bytesCount) {
  ...
  if (bytesCount != 0) this->resize(bytesCount);
}

GpuMemory::~GpuMemory() { this->clear(); }

void GpuMemory::resize(size_t bytesCount) {
  if (bytesCount <= m_bytesCount) return;
  ...
  VkBufferCreateInfo bufferInfo{.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
                                .size = bytesCount,
                                .usage = m_usage};
  r = vkCreateBuffer(m_device, &bufferInfo, nullptr, &m_buf);
  if (r != VK_SUCCESS) throw std::runtime_error("Failed to create buffer");
  ...
  VkMemoryAllocateInfo allocInfo{
      ...
      .memoryTypeIndex = findMemoryType(
          ...
          VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
              VK_MEMORY_PROPERTY_HOST_COHERENT_BIT),  // getHostVisibleMemoryIndex(m_physicalDevice)
  };
  r = vkAllocateMemory(m_device, &allocInfo, nullptr, &m_bufMem);
  if (r != VK_SUCCESS) throw std::runtime_error("Failed to allocate memory");

  r = vkBindBufferMemory(m_device, m_buf, m_bufMem, 0);
  if (r != VK_SUCCESS) {
    auto msg =
        std::format("Failed to bind buffer memory: {}", static_cast<int>(r));
...
GpuMemory::Mapping GpuMemory::access() {
  if (m_bytesCount == 0) return Mapping();
...
GpuMemory::Mapping::~Mapping() { vkUnmapMemory(m_dev, m_bufMem); }

std::byte* GpuMemory::Mapping::data(size_t offset, size_t size) {
  // vkUnmapMemory(m_dev, m_bufMem);

  std::byte* data;
  VkResult r = vkMapMemory(m_dev, m_bufMem, offset, size, 0,
                           reinterpret_cast<void**>(&data));
  if (r != VK_SUCCESS) {
    auto msg = std::format("Failed to map memory: {}", static_cast<int>(r));
    throw std::runtime_error(msg);
  }
  return data;
}
...
VkDeviceSize GpuMemory::uniformBlockSize(VkDeviceSize bytesCount) const {
  static std::unordered_map<VkPhysicalDevice, VkPhysicalDeviceProperties>
      s_cachedProperties;
  auto it = s_cachedProperties.find(m_physicalDevice);
  if (it == s_cachedProperties.end()) {
    VkPhysicalDeviceProperties properties;
    vkGetPhysicalDeviceProperties(m_physicalDevice, &properties);
    it = s_cachedProperties.insert(std::make_pair(m_physicalDevice, properties))
             .first;
  }
