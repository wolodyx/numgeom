...
double GetIsolineCurveLength(Handle(Geom_BoundedSurface) surface,
                             Standard_Boolean isUDirection, double param) {
  double u1, u2, v1, v2;
  surface->Bounds(u1, u2, v1, v2);

  gp_Pnt2d orig(isUDirection ? u1 : param, isUDirection ? param : v1);
  gp_Dir2d dir(isUDirection ? 1.0 : 0.0, isUDirection ? 0.0 : 1.0);
  Standard_Real uFirst = isUDirection ? v1 : u1;
  Standard_Real uLast = isUDirection ? v2 : u2;

  Handle(Adaptor2d_Line2d) isoline =
      new Adaptor2d_Line2d(orig, dir, uFirst, uLast);
...
void ExtendSurfacesOfExternalFaces(
  ...
  for (auto it = externalFaces.cbegin(); it != externalFaces.cend(); ++it) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (inU && boundedSurface->IsUPeriodic()) continue;
      if (!inU && boundedSurface->IsVPeriodic()) continue;
...
namespace {
;
void GetCommonEdges(const TopoDS_Face& f1, const TopoDS_Face& f2,
...
FaceFaceIntersections::FFResult FaceFaceIntersections::GetIntersections(
    const TopoDS_Face& fFirst) const {
  auto it = myFace2intersections.find(fFirst);
  if (it != myFace2intersections.end()) return FFResult(it->second);
...
FaceFaceIntersections::CFResult FaceFaceIntersections::GetIntersections(
  ...
  if (it != myCurve2intersections.end()) return CFResult(it->second);
...
void CalcSpecialPoints(const TopoDS_Face& F,
...
Standard_Boolean IsIntersected(const Handle(Geom_BoundedCurve) & C,
  ...
  for (auto it = faces.cbegin(); it != faces.cend(); ++it) {
    ...
    // Определяем список точек на грани, касания с которыми разрешается концам
    // кривой.
    std::vector<SurfaceDomain> specialPoints;
    ...
    for (Standard_Integer i = 1; i <= nbPoints; ++i) {
      intCS.Parameters(i, u, v, t);
      gp_Pnt2d uv(u, v);
      if (OnFace(F, uv) && !Has(uv, specialPoints)) return Standard_True;
    }
    ...
    for (Standard_Integer i = 1; i <= nbSegments; ++i) {
      ...
      Handle(Geom_BoundedCurve) C2 =
          Handle(Geom_BoundedCurve)::DownCast(intCS.Segment(i));
      assert(!C2.IsNull());
      Handle(Geom2d_BoundedCurve) projCurve = Project(C2, S);
      if (IsIntersected(projCurve, F)) return Standard_True;
    }
  }
...
Standard_Boolean IsIntersected(const Handle(Geom2d_BoundedCurve) & curve,
  ...
  for (TopExp_Explorer it(face, TopAbs_EDGE); it.More(); it.Next()) {
    ...
    if (intCC.NbSegments() != 0) return Standard_True;
  }
  return Standard_False;
}
