...
NUMGEOM_EXPORT void Tesselate(const Handle(Geom_Curve) & C,
                              Standard_Real uFirst, Standard_Real uLast,
                              std::vector<gp_Pnt>& polyline,
                              Standard_Real relAccuracy = 0.01);

NUMGEOM_EXPORT void Tesselate(const Handle(Geom2d_Curve) & C,
                              Standard_Real uFirst, Standard_Real uLast,
                              std::vector<gp_Pnt2d>& polyline,
                              Standard_Real relAccuracy = 0.01);

NUMGEOM_EXPORT void Tesselate(const TopoDS_Face& F, const TopoDS_Wire& W,
                              std::vector<gp_Pnt2d>& polyline,
                              Standard_Real relAccuracy = 0.01);
