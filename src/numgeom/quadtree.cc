...
struct QuadTree::Internal {
  ...
  void GetCellsOfNextLevel(const Cell& parentCell,
                           std::array<Standard_Integer, 4>& cells) const;

  //! Размерность подложки уровня `level`.
  void LevelSize(Standard_Integer level, Standard_Integer& ni,
                 Standard_Integer& nj) const;

  Standard_Integer GetNbCellsOnLevel(Standard_Integer level) const;

  void GetCellCoords(const Cell& cell, Standard_Integer& i,
                     Standard_Integer& j) const;

  void NodeIndex2Coords(const Node& node, Standard_Integer& i,
                        Standard_Integer& j) const;
  ...
  gp_Pnt2d PointCoords(const Node& node) const;

  Node Normalize(const Node& node) const;

  void Nodes(const Cell& cell, std::array<Node, 4>& nodes) const;
};
...
QuadTree::Cell QuadTree::Internal::GetChildCell(
    const Cell& parentCell, Standard_Integer childIndex) const {
  ...
  switch (childIndex) {
    case 0:
      return Cell{parentCell.level + 1, j * ni + i};
    case 1:
      return Cell{parentCell.level + 1, j * ni + i + 1};
    case 2:
      return Cell{parentCell.level + 1, (j + 1) * ni + i + 1};
    case 3:
      return Cell{parentCell.level + 1, (j + 1) * ni + i};
  }
  return Cell();
}

void QuadTree::Internal::GetCellsOfNextLevel(
    const Cell& parentCell, std::array<Standard_Integer, 4>& cells) const {
  Standard_Integer ni, nj;
  this->LevelSize(parentCell.level, ni, nj);
...
//! Размерность подложки уровня `level`.
void QuadTree::Internal::LevelSize(Standard_Integer level, Standard_Integer& ni,
                                   Standard_Integer& nj) const {
  assert(level >= 0);
  ni = nbByI * (0x01 << level);
  nj = nbByJ * (0x01 << level);
...
void QuadTree::Internal::GetCellCoords(const Cell& cell, Standard_Integer& i,
                                       Standard_Integer& j) const {
  Standard_Integer ni, nj;
  this->LevelSize(cell.level, ni, nj);
  i = cell.index % ni;
...
void QuadTree::Internal::NodeIndex2Coords(const Node& node, Standard_Integer& i,
                                          Standard_Integer& j) const {
  Standard_Integer ni, nj;
...
Standard_Boolean QuadTree::Internal::IsExists(const Cell& cell) const {
  ...
  if (beNonTerminal) {
    ...
    do {
      ...
      if (level >= nonTerminalCells.size()) break;
                                ...
                                nonTerminalCells[level].end(), i + j * ni));
    ...
    while (true) {
      ...
      Standard_Boolean beNonTerminalParentCell = std::binary_search(
          nonTerminalCells[level - 1].begin(),
          nonTerminalCells[level - 1].end(), i / 2 + j / 2 * ni / 2);
...
QuadTree::Cell QuadTree::Internal::Next(const Cell& startCell) const {
  ...
  while (true) {
    if (cell.level == 0) {
      ...
      if (nextCellIndex == nbByI * nbByJ) return Cell{0, nextCellIndex};
...
gp_Pnt2d QuadTree::Internal::PointCoords(const Node& node) const {
  Standard_Integer ni, nj;
  this->LevelSize(node.level, ni, nj);
  Standard_Integer i, j;
...
QuadTree::Node QuadTree::Internal::Normalize(const Node& node) const {
  ...
  if (node.level != 0) {
    while (i != 0 && j != 0 && i % 2 == 0 && j % 2 == 0 && ni % 2 == 0 &&
           nj % 2 == 0) {
      i /= 2, j /= 2, ni /= 2, nj /= 2;
      ++levelDown;
    }
...
void QuadTree::Internal::Nodes(const Cell& cell,
                               std::array<Node, 4>& nodes) const {
  Standard_Integer ni, nj, i, j;
  this->GetCellCoords(cell, i, j);
  this->LevelSize(cell.level, ni, nj);
  ni += 1, nj += 1;
...
QuadTree::QuadTree(const Bnd_Box2d& boundBox, Standard_Integer in,
                   Standard_Integer jn) {
  pimpl = new Internal();
...
QuadTree::~QuadTree() { delete pimpl; }

Standard_Integer QuadTree::Levels() const { return pimpl->Levels(); }

void QuadTree::GetBox(Standard_Real& xMin, Standard_Real& yMin,
...
namespace {
;
Standard_Integer CellsCount(const QuadTree::Internal* qTreeInternal,
...
class IteratorImpl_QuadTreeCells : public IteratorImpl<QuadTree::Cell> {
 ...
 public:
  IteratorImpl_QuadTreeCells(
    ...
    if (!myCurrentCell)
      myCurrentCell =
          myQTreeInternal->GetFirstTerminalCell(QuadTree::Cell{0, 0});
  }
  ...
  QuadTree::Cell current() const override { return myCurrentCell; }
  ...
  IteratorImpl<QuadTree::Cell>* last() const override {
    ...
    return new IteratorImpl_QuadTreeCells(myQTreeInternal,
                                          QuadTree::Cell{0, N});
  }
...
class IteratorImpl_QuadTreeCellsOfLevel : public IteratorImpl<QuadTree::Cell> {
 ...
 public:
  IteratorImpl_QuadTreeCellsOfLevel(const QuadTree::Internal* qTreeInternal,
    ...
    if (!myQTreeInternal->IsTerminal(cell)) GoToNextTerminalCell();
  }
  ...
  IteratorImpl<QuadTree::Cell>* clone() const override {
    return new IteratorImpl_QuadTreeCellsOfLevel(myQTreeInternal, myLevel,
                                                 myCurrentIndex);
  }
...
class IndexIterator {
 ...
 public:
  ...
  IndexIterator(const QuadTree::Internal* qTreeInternal,
                const QuadTree::Cell& cell, Standard_Integer upLevel) {
    myUpLevel = upLevel;
  ...
  IndexIterator& operator++() {
    ...
    Standard_Boolean beOnSide[4] = {
        myCurrentJ == jIndexMin, myCurrentI == iIndexMax,
        myCurrentJ == jIndexMax, myCurrentI == iIndexMin};
  ...
  Standard_Boolean More() const { return myCurrentI != -1 || myCurrentJ != -1; }

  Standard_Boolean Empty() const { return myICell == -1 && myJCell == -1; }
  ...
  Standard_Boolean operator==(const IndexIterator& other) const {
    return myICell == other.myICell && myJCell == other.myJCell &&
           myCurrentI == other.myCurrentI && myCurrentJ == other.myCurrentJ &&
...
class IteratorImpl_ConnectedCells : public IteratorImpl<QuadTree::Cell> {
 ...
 public:
  IteratorImpl_ConnectedCells(
      const QuadTree::Internal* qTreeInternal, const QuadTree::Cell& cell,
    ...
    if (myIndexIterator.Empty())
      myIndexIterator =
          IndexIterator(qTreeInternal, cell, qTreeInternal->Levels() - 1);
  ...
  IteratorImpl<QuadTree::Cell>* clone() const override {
    return new IteratorImpl_ConnectedCells(myQTreeInternal, myBaseCell,
                                           myIndexIterator);
  }
...
void QuadTree::GetCellBox(const Cell& cell, Standard_Real& xMin,
                          Standard_Real& yMin, Standard_Real& xMax,
...
Standard_Boolean QuadTree::Dump(const std::filesystem::path& fileName) const {
  ...
  for (Standard_Integer i = 0; i < nbCells; ++i)
    file << "4 " << 4 * i << ' ' << 4 * i + 1 << ' ' << 4 * i + 2 << ' '
         << 4 * i + 3 << std::endl;
  file << std::endl;

  file << "CELL_TYPES " << nbCells << std::endl;
  for (size_t i = 0; i < nbCells; ++i) file << "9 ";  //< VTK_QUAD
  file << std::endl;
  return Standard_True;
}

void QuadTree::CellsNumberOnLevel(Standard_Integer level,
                                  Standard_Integer& nbByI,
                                  Standard_Integer& nbByJ) const {
...
void QuadTree::GetCellCoords(const Cell& cell, Standard_Integer& i,
                             Standard_Integer& j) const {
  pimpl->GetCellCoords(cell, i, j);
}
...
QuadTree::Cell QuadTree::GetCell(const gp_Pnt2d& Q) const {
  ...
  if (xQ < xMin || xQ > xMax || yQ < yMin || yQ > yMax) return Cell();
  ...
  for (Standard_Integer level = 0;; ++level) {
    ...
    Standard_Integer i =
        static_cast<Standard_Integer>(std::floor((xQ - xMin) / xStep));
    Standard_Integer j =
...
QuadTree::Ptr QuadTree::Deserialize(std::istream& stream) {
  ...
  Bnd_Box2d boundBox;
  {
    ...
    if (!jBoundBox.is_array() || jBoundBox.size() != 4) return QuadTree::Ptr();
    Standard_Real xMin, yMin, xMax, yMax;
...
QuadTree::Ptr QuadTree::Create(const Bnd_Box2d& boundBox, Standard_Integer in,
                               Standard_Integer jn) {
  if (in < 1 || jn < 1) return QuadTree::Ptr();
...
namespace {
;
QuadTree::Cell GetParentCell(QuadTree::Internal* qTreeIntenal,
...
QuadTree::CellConnection QuadTree::GetConnectionType(const Cell& aCell,
  ...
  if (ja == jb) {
    if (ia == ib) return CellConnection::Inner;
    if (ia == ib - 1) return CellConnection::RightSide;
    if (ia == ib + 1) return CellConnection::LeftSide;
    return CellConnection::Outer;
  } else if (ja == jb - 1) {
    if (ia == ib) return CellConnection::UpSide;
    if (ia == ib - 1) return CellConnection::UpRightCorner;
    if (ia == ib + 1) return CellConnection::UpLefttCorner;
    return CellConnection::Outer;
  } else if (ja == jb + 1) {
    if (ia == ib) return CellConnection::DownSide;
    if (ia == ib - 1) return CellConnection::DownRightCorner;
    if (ia == ib + 1) return CellConnection::DownLeftCorner;
    return CellConnection::Outer;
  }
  return CellConnection::Outer;
}

void QuadTree::GetMinMaxDistances(const gp_Pnt2d& q, const Cell& cell,
...
struct CellWithDistances {
  ...
  CellWithDistances(QuadTree::CPtr qTree, const QuadTree::Cell& c,
                    const gp_Pnt2d& q)
      : cell(c) {
...
void PushNonVisitedCellsToQueue(QuadTree::CPtr qTree,
...
void SearchNearestCells(
    ...
    Standard_Real maximumDistance, std::list<QuadTree::Cell>& nearestCells) {
  nearestCells.clear();
