...
Standard_Boolean Dump(const std::string& fileName,
...
TopoDS_Edge CreateEdge(Handle(Geom_Curve) curve, const TopoDS_Vertex& vBeg,
                       const TopoDS_Vertex& vEnd);

void RemoveFaces(const TopoDS_Shape& initShape,
...
TopoDS_Shape CreateResult(const TopTools_ListOfShape& visibleFaces);
...
TopoDS_Shape RemoveFaces(const TopoDS_Shape& initShape,
  ...
  if (initShape.IsNull()) return TopoDS_Shape();

  TopTools_IndexedDataMapOfShapeListOfShape vertex2edges;
  TopExp::MapShapesAndAncestors(initShape, TopAbs_VERTEX, TopAbs_EDGE,
                                vertex2edges);

  TopTools_IndexedDataMapOfShapeListOfShape edge2faces;
  TopExp::MapShapesAndAncestors(initShape, TopAbs_EDGE, TopAbs_FACE,
                                edge2faces);

  // Разделение граней на сильно связанные компоненты.
  BOPTools_ListOfConnexityBlock features;
  BOPTools_AlgoTools::MakeConnexityBlocks(facesToRemove, TopAbs_EDGE,
                                          TopAbs_FACE, features);

  TopTools_ListOfShape visibleFaces;
  for (const BOPTools_ConnexityBlock& cblock : features) {
    RemoveFaces(initShape, cblock, vertex2edges, edge2faces, visibleFaces);
  }
...
struct VisibleContour {
  ...
  */
  struct Piece {
    ...
    TopoDS_Face F;
    Handle(Geom_Curve) C;
    // Параметры точек на кривой, где участок контура начинается и
    // заканчивается.
  ...
  void Close() {
    ...
    if (m_isClosedContour) pt = solution.front().BegPoint();
  ...
  Piece PopBack() {
    ...
    solution.pop_back();
    if (!solution.empty()) solution.back().tMax = HUGE_VAL;
    return p;
  }
 ...
 private:
  Standard_Boolean IsValid() const {
    if (solution.size() < 2) return Standard_True;

    size_t n = solution.size();
    size_t N = n;
    if (!m_isClosedContour || solution.back().tMax == HUGE_VAL) n = n - 1;
    for (size_t i = 0; i < n; ++i) {
      ...
      if (d2 > 1.0e-7) return Standard_False;
    }
    return Standard_True;
  }

  gp_Pnt Intersect(const Handle(Geom_Curve) & C,
                   const Handle(Geom_Surface) & S) const {
    GeomAPI_IntCS csInt(C, S);
    if (!csInt.IsDone()) return gp_Pnt(HUGE_VAL, HUGE_VAL, HUGE_VAL);
    return csInt.Point(1);
  }
};

// Создаем восстанавливающую грань по найденному решению.
TopoDS_Face CreateVisibleFace(const Handle(Geom_Surface) & sEx,
  ...
  if (isClosedContour) {
    ...
    for (auto it = itc; it != itc2; ++it) wireMaker.Add(TopoDS::Edge(*it));
  ...
  if (!innerWire.IsNull()) faceMaker.Add(innerWire);
...
TopoDS_Face GetAdjFace(
    const TopoDS_Face& face, const TopoDS_Edge& edge,
    const TopTools_IndexedDataMapOfShapeListOfShape& edge2faces) {
  ...
  if (face1 == face) return TopoDS::Face(face2);
  return TopoDS::Face(face1);
}

TopoDS_Face GetNextExternalFace(
    const TopoDS_Face& fEx, const TopoDS_Vertex& vertex,
    const TopTools_IndexedDataMapOfShapeListOfShape& edge2faces,
...
Standard_Boolean IsFaceInnerWire(const TopoDS_Face& F,
  ...
  for (const auto& s : wire.solution) {
    ...
    if (IsIntersected(bC2, F)) return Standard_False;
  }

  return Standard_True;
}

Standard_Boolean IsValidClosedContour(const TopoDS_Face& exFace,
                                      const VisibleContour& contour) {
  return IsFaceInnerWire(exFace, contour);
}

Standard_Boolean IsSelfIntersected(const TopoDS_Face& F,
  ...
  for (auto it = solution.solution.begin(); it != solution.solution.end() - 1;
       ++it) {
    const auto& s = (*it);
...
class SearchStack {
 public:
  ...
  void Append(const ContourSeed& seed) { stack.top().push(seed); }
...
TopoDS_Face RecoverVisibleFace(
    const TopoDS_Shape& initShape, const TopoDS_Face& fEx,
    const TopTools_MapOfShape& externalFaces,
  ...
  if (!isClosedContour) std::tie(vBeg, vEnd) = GetBoundaryVertices(itc, itc2);

  SearchStack sStack;
  VisibleContour solution(isClosedContour,
                          isClosedContour ? gp_Pnt() : BRep_Tool::Pnt(vEnd));
  ...
  if (isClosedContour) {
    ...
    for (auto ffInt : ffIntersections.GetIntersections(fEx)) {
      for (auto cfInt : ffIntersections.GetIntersections(ffInt.C)) {
        if (cfInt.F == fEx || cfInt.F == ffInt.F) continue;
    ...
    sStack.AddNextState();
    TopoDS_Face fexStart =
        GetNextExternalFace(fEx, vBeg, edge2faces, bvertex2eedges);
  ...
  while (!sStack.IsEmpty()) {
    ...
    if (isClosedContour && solution.IsEmpty()) fexFinish = seed.F;
    ...
    if (!emptySolution && seed.F == fexFinish) {
      if (isClosedContour) solution.PopBack();

      solution.Close();
      if (isClosedContour) {
        if (IsValidClosedContour(fEx, solution)) break;
      } else {
        break;
      }
      continue;
    ...
    for (const auto& ints : ffIntersections.GetIntersections(ffCurve)) {
      if (ints.F == seed.F) continue;

      gp_Pnt P = ffCurve->Value(ints.t);
      if (seed.P.Distance(P) <= 1.e-7) continue;

      if (exfsProcessed.Contains(ints.F)) continue;

      sStack.Append({ints.F, P});
    }

    if (sStack.IsStateEmpty()) continue;

    if (seed.F != fexFinish) exfsProcessed.Add(seed.F);
  }

  if (solution.IsEmpty()) return TopoDS_Face();

  // Создаем восстанавливающую грань по найденному решению.
  TopoDS_Face fSol =
      CreateVisibleFace(sEx, isClosedContour, solution, itc, itc2);
  return fSol;
}
...
void RemoveFaces(const TopoDS_Shape& initShape,
  ...
  TopTools_MapOfShape externalFaces;
  TopTools_IndexedDataMapOfShapeListOfShape bvertex2eedges;
  ExtractBoundaryShapes(cblock.Shapes(), vertex2edges, edge2faces, contours,
                        externalFaces, bvertex2eedges);
  ...
  for (const auto& contour : contours) {
    ...
    while (itc != contour.end()) {
      TopoDS_Face fEx;
      auto itc2 = GetNextContourPiece(itc, contour.end(), externalFaces,
                                      edge2faces, fEx);
      STATE_DIAGNOSTIC(!fEx.IsNull());
      STATE_DIAGNOSTIC(itc != itc2);

      TopoDS_Face slf =
          RecoverVisibleFace(initShape, fEx, externalFaces, ffIntersections,
                             contour, itc, itc2, edge2faces, bvertex2eedges);
...
bool IsBoundary(const TopoDS_Edge& edge,
                const TopTools_IndexedDataMapOfShapeListOfShape& edge2faces,
                const TopTools_ListOfShape& faces, TopoDS_Face& externalFace) {
  ...
  for (TopExp_Explorer itw(face, TopAbs_WIRE); itw.More(); itw.Next()) {
    ...
    for (BRepTools_WireExplorer ite(wire); ite.More(); ite.Next()) {
      ...
      if (prevEdge == edge) {
        while (true) {
          ite.Next();
          if (!ite.More()) ite.Init(wire);
          TopoDS_Edge eNext = TopoDS::Edge(ite.Current());
          if (!BRep_Tool::Degenerated(eNext)) return eNext;
        }
      }
    }
  }
...
TopoDS_Edge GetOutgoingEdge(const TopoDS_Face& face,
  ...
  for (TopExp_Explorer itw(face, TopAbs_WIRE); itw.More(); itw.Next()) {
    ...
    for (BRepTools_WireExplorer ite(wire); ite.More(); ite.Next()) {
      ...
      if (BRep_Tool::Degenerated(edge)) continue;
      if (vertex.IsSame(GetBegVertex(edge))) return edge;
    }
  }
  return TopoDS_Edge();
}
...
Standard_Boolean CheckContours(
  ...
  for (const TopTools_ListOfShape& contour : contours) {
    if (contour.Size() == 1) {
      ...
      if (IsClosed(edge)) return Standard_False;
      continue;
    }
    ...
    for (const TopoDS_Shape& E : contour) {
      ...
      if (!vBeg.IsSame(vPre)) return Standard_False;
      vPre = vEnd;
    }
  }
  ...
  for (const TopTools_ListOfShape& contour : contours) {
    for (const TopoDS_Shape& edge : contour) contourEdges.Add(edge);
  }
  ...
  for (const TopTools_ListOfShape& contour : contours) {
    ...
    if (bvertex2eedges.Contains(vFirst)) continue;
    ...
    for (++it; it != contour.cend(); ++it) {
      ...
      if (bvertex2eedges.Contains(v)) return Standard_False;
    }
  }
...
Standard_Boolean Joinable(const TopTools_ListOfShape& contour,
                          const TopoDS_Edge& nextEdge) {
  if (contour.IsEmpty()) return Standard_True;
...
void GetAdjacentEntities(
    ...
    const TopTools_IndexedDataMapOfShapeListOfShape& edge2faces,
    TopTools_MapOfShape& boundaryEdges, TopTools_MapOfShape& internalEdges,
    TopTools_MapOfShape& externalFaces) {
...
void ExtractBoundaryShapes(
  ...
  GetAdjacentEntities(inputFaces, edge2faces, boundaryEdges, internalEdges,
                      externalFaces);
  ...
  for (const auto& f : inputFaces) nonProcessedFaces.Add(f);
  while (!nonProcessedFaces.IsEmpty()) {
    ...
    for (TopExp_Explorer it(cFace, TopAbs_EDGE); it.More(); it.Next()) {
      ...
      if (!boundaryEdges.Contains(edge)) continue;

      if (processedBoundaryEdges.Contains(edge)) continue;
  ...
  for (const TopTools_ListOfShape& contour : contours) {
    for (const TopoDS_Shape& edge : contour) {
      ...
      for (auto it = edges.begin(); it != edges.end(); ++it) {
        ...
        if (boundaryEdges.Contains(e) || internalEdges.Contains(e)) continue;

        TopoDS_Edge eExternal = TopoDS::Edge(e);
        if (!GetEndVertex(eExternal).IsSame(boundaryVertex))
          eExternal.Reverse();
        if (!externalEdges.Contains(eExternal)) externalEdges.Append(eExternal);
      }
  ...
  for (TopTools_ListOfShape& contour : contours) {
    ...
    int n = 0;
    bool has = false;
    for (TopTools_ListOfShape::Iterator it(contour); it.More();
         it.Next(), ++n) {
    ...
    if (n != 0 && contour.Size() != n) Rotate(contour, n);
  }

  STATE_DIAGNOSTIC(CheckContours(contours, boundaryEdges, bvertex2eedges));
}
...
std::pair<TopoDS_Vertex, TopoDS_Vertex> GetBoundaryVertices(
  ...
  auto itLast = itBegin;
  for (auto it = itBegin; it != itEnd; ++it) itLast = it;
...
TopoDS_Shape CreateResult(const TopTools_ListOfShape& visibleFaces) {
  ...
  for (const TopoDS_Shape& face : visibleFaces) builder.Add(result, face);
  return result;
}
...
Standard_Boolean Dump(const std::string& fileName,
  ...
  std::unordered_map<TopoDS_Shape,
                     std::pair<Handle(Poly_Triangulation), TopLoc_Location>>
      face2trng;
  ...
  file << "CELLS " << nbCells << ' ' << 4 * nbTrias + 3 * nbLines + 2 * nbVerts
       << std::endl;
  ...
  for (auto it = allFaces.cbegin(); it != allFaces.cend(); ++it) {
    ...
    for (Standard_Integer i = 1; i <= nbTrias; ++i) {
      ...
      file << "3 " << tr.Value(1) - 1 + offset << ' '
           << tr.Value(2) - 1 + offset << ' ' << tr.Value(3) - 1 + offset
           << ' ';
    }
    file << std::endl;
    offset += trng->NbNodes();
  ...
  for (Standard_Integer i = 0; i < nbTrias; ++i) file << "5 ";
  file << std::endl;
  for (Standard_Integer i = 0; i < nbLines; ++i) file << "3 ";
  for (Standard_Integer i = 0; i < nbVerts; ++i) file << "1 ";
  file << std::endl;
  ...
  for (auto it = allFaces.cbegin(); it != allFaces.cend(); ++it) {
    ...
    for (Standard_Integer i = 0; i < N; ++i) file << id << ' ';
    ++id;
    file << std::endl;
  }
  for (auto it = allEdges.cbegin(); it != allEdges.cend(); ++it) {
    const Poly_ArrayOfNodes& polyline = edge2trng.at(*it);
    Standard_Integer nbLines = polyline.Size() - 1;
    for (Standard_Integer i = 0; i < nbLines; ++i) file << id << ' ';
    ++id;
    file << std::endl;
  }
  for (Standard_Integer i = 0; i < nbVerts; ++i) file << id << ' ';
  file << std::endl;
  file << std::endl;
  ...
  for (auto it = allFaces.cbegin(); it != allFaces.cend(); ++it) {
    ...
    Standard_Integer N = trng->NbTriangles();
    for (Standard_Integer i = 1; i <= N; ++i) file << ch << ' ';
  }
  for (Standard_Integer i = 0; i < nbLines + nbVerts; ++i) file << '0' << ' ';
  file << std::endl;

  file << "SCALARS contour char 1" << std::endl;
  file << "LOOKUP_TABLE default" << std::endl;
  for (Standard_Integer i = 0; i < nbTrias; ++i) file << '0' << ' ';
  for (auto it = allEdges.cbegin(); it != allEdges.cend(); ++it) {
    const auto& E = (*it);
    char ch =
        (contour.Contains(E) || contour.Contains(E.Reversed()) ? '1' : '0');
    const Poly_ArrayOfNodes& polyline = edge2trng.at(E);
    Standard_Integer nbLines = polyline.Size() - 1;
    for (Standard_Integer i = 0; i < nbLines; ++i) file << ch << ' ';
  }
  for (Standard_Integer i = 0; i < nbVerts; ++i) file << '0' << ' ';
...
bool VisibleContour::Dump(const std::string& fileName) const {
  ...
  for (const auto& s : this->solution) {
    ...
    if (s.tMax == HUGE_VAL) {
      ...
      if (boundedCurve)
        ...
        GCPnts_AbscissaPoint abscissaPoint(GeomAdaptor_Curve(s.C), length,
                                           s.tMin);
  ...
  file << "CELL_TYPES " << nbCells << std::endl;
  for (Standard_Integer i = 0; i < nbCells; ++i) file << '3' << ' ';
