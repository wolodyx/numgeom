...
void Tesselate(const Handle(Geom_Curve) & C, Standard_Real uFirst,
               Standard_Real uLast, std::vector<gp_Pnt>& polyline,
...
void Tesselate(const Handle(Geom2d_Curve) & C, Standard_Real uFirst,
               Standard_Real uLast, std::vector<gp_Pnt2d>& polyline,
               Standard_Real relAccuracy) {
...
void Tesselate(const TopoDS_Face& F, const TopoDS_Wire& W,
               std::vector<gp_Pnt2d>& polyline, Standard_Real relAccuracy) {
  ...
  for (BRepTools_WireExplorer it(W); it.More(); it.Next()) {
    ...
    if (E.Orientation() == TopAbs_REVERSED) std::swap(u0, u1);
    Tesselate(C, u0, u1, pl);
    auto itBeg = pl.begin();
    if (!polyline.empty()) itBeg = std::next(itBeg);
