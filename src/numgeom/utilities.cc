...
Bnd_Box ComputeBoundBox(const TopoDS_Shape& shape) {
  if (shape.IsNull()) return Bnd_Box();
...
Standard_Boolean ReadFromFile(const std::filesystem::path& fileName,
                              TopoDS_Shape& shape) {
  if (!std::filesystem::exists(fileName)) return Standard_False;
  ...
  if (ext == ".step" || ext == ".stp") {
    ...
    if (reader.ReadFile(str.c_str()) != IFSelect_RetDone) return Standard_False;
...
Standard_Boolean IsWaterproof(const TopoDS_Shape& shape) {
  ...
  for (Standard_Integer i = 1; i <= nEdges; ++i) {
    ...
    if (nOwningFaces != 2) return false;
  }
  return true;
}
...
Standard_Boolean OnFace(const TopoDS_Face& F, const gp_Pnt2d& uv) {
  ...
  if (state == TopAbs_ON || state == TopAbs_IN) return Standard_True;
  return Standard_False;
}

Handle(Geom2d_BoundedCurve) Project(const Handle(Geom_BoundedCurve) & curve,
...
Standard_Real Project(const gp_Pnt2d& P, const Handle(Geom2d_Curve) & C) {
  ...
  if (project.NbPoints() == 0) return HUGE_VAL;
...
namespace {
;
void ExtractTriangulableShapes(const TopoDS_Shape& initShape,
                               std::vector<TopoDS_Shape>& resultShapes) {
  if (initShape.IsNull()) return;
...
TriMesh::Ptr ConvertToTriMesh(const TopoDS_Shape& initShape) {
  ...
  for (const auto& v : face2triangulation) {
    ...
    for (Standard_Integer i = 1; i <= nbCells; ++i) {
      ...
      cell.Get(na, nb, nc);
      if (!normalConsistent) std::swap(nb, nc);
      mesh->GetCell(cellIndex++) =
          TriMesh::Cell{static_cast<TriMesh::IndexType>(na + nodeOffset - 1),
...
Standard_Integer RoundToInt(Standard_Real value) {
  ...
  if (value < 0.0) return static_cast<Standard_Integer>(std::ceil(value - 0.5));
  return 0;
}
}  // namespace

void GetMeshSideSizes(const Bnd_Box2d& box, Standard_Integer nbCells,
                      Standard_Integer& xN, Standard_Integer& yN) {
  ...
  for (Standard_Integer iter = 0; iter < 16; ++iter) {
    ...
    if (n == nbCells || n1Max - n1Min < 2) break;
  ...
  if (xWidth != maxWidth) std::swap(n1, n2);
  xN = n1, yN = n2;
}

void GetMeshSideSizes(const Bnd_Box& box, Standard_Integer nbCells,
                      Standard_Integer& xN, Standard_Integer& yN,
                      Standard_Integer& zN) {
  Standard_Real xMin, yMin, zMin, xMax, yMax, zMax;
  box.Get(xMin, yMin, zMin, xMax, yMax, zMax);

  Standard_Real width[3] = {xMax - xMin, yMax - yMin, zMax - zMin};
  ...
  for (Standard_Integer iter = 0; iter < 16; ++iter) {
    ...
    if (maxWidthIndex != 0) nn[0] = RoundToInt(width[0] / h);
    if (maxWidthIndex != 1) nn[1] = RoundToInt(width[1] / h);
    if (maxWidthIndex != 2) nn[2] = RoundToInt(width[2] / h);

    Standard_Integer vol = nn[0] * nn[1] * nn[2];

    if (vol == nbCells || nMax - nMin < 2) break;
