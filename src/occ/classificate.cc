...
enum PositionType { OuterPosition, InnerPosition, BoundaryPosition };
...
class MeshLinesAndContoursIntersections {
 ...
 public:
  MeshLinesAndContoursIntersections(const QuadTree&, const FaceContours&,
...
void CalcLineCoefficients(const gp_Pnt2d&, const gp_Pnt2d&, Standard_Real& A,
                          Standard_Real& B, Standard_Real& C);

Standard_Boolean PointInsideBox(const gp_Pnt2d& P, Standard_Real xMin,
...
Standard_Boolean ContourLieInnerCell(const FaceContours&, Standard_Real xMin,
                                     Standard_Real yMin, Standard_Real xMax,
                                     Standard_Real yMax);

PositionType ClassificateCell(const QuadTree& qTree, const QuadTree::Cell& cell,
...
Standard_Boolean IsBoundaryCell(QuadTree::CPtr qTree,
...
*/
Standard_Real NearestDistance(const gp_Pnt2d& p,
  ...
  if (!c1ptr || !c2ptr) return std::numeric_limits<Standard_Real>::max();
...
*/
Standard_Real NearestDistance2(const gp_Pnt2d& p,
                               const CircularList<ContourPoint>::Node* cptr) {
  ...
  for (Standard_Integer i = 0; i < 2; ++i) {
    ...
    if (t < 0.0 - tol || t > 1.0 + tol) continue;
    Standard_Real dist = ap.Crossed(w) / w.Magnitude();
    if (std::abs(dist) < std::abs(minDistance)) minDistance = dist;
  }
  return minDistance;
}

TopAbs_State Classificate(const TopoDS_Face& F, const gp_Pnt2d& Q,
                          const AuxClassificateData2d** pAux) {
  if (F.IsNull()) return TopAbs_UNKNOWN;
  ...
  if (pAux) {
    if (!(*pAux)) (*pAux) = new AuxClassificateData2d(F, Tol);
  } else {
    if (!s_aux || s_aux->face != F) s_aux = new AuxClassificateData2d(F, Tol);
  }
  ...
  for (const QuadTree::Cell& cell : boundaryCells) {
    ...
    if (std::abs(qcDistance) <= aux->tolerance) return TopAbs_ON;
...
Standard_Boolean FaceContours::Dump(
  ...
  for (const auto& pl : myWires) {
    ...
    for (const auto& cp : pl) polyline.push_back(cp.p);
...
Standard_Boolean IntersectSegmentSegment(const gp_Pnt2d& seg1p1,
  ...
  */
  ...
  if (std::abs(D) <= 1.e-7) return Standard_False;
...
void MeshLinesAndContoursIntersections::IntersectVertical(
    Standard_Real xConst, std::vector<Cross>& inter) const {
  ...
  for (const CircularList<ContourPoint>& contour : myContours.myWires) {
    for (auto it = contour.begin(); it != contour.end(); ++it) {
      ...
      Standard_Boolean b1 =
          (xConst >= x0 || xConst >= x1) && (xConst <= x0 || xConst <= x1);
      Standard_Boolean b2 =
  ...
  struct Less {
    bool operator()(const Cross& a, const Cross& b) const { return a.u < b.u; }
  };
  std::sort(inter.begin(), inter.end(), Less());
}

void MeshLinesAndContoursIntersections::IntersectHorizontal(
    Standard_Real yConst, std::vector<Cross>& inter) const {
  ...
  for (const auto& polyline : myContours.myWires) {
    for (auto it = polyline.begin(); it != polyline.end(); ++it) {
      ...
      Standard_Real y0 = p0.Y(), y1 = p1.Y();
      Standard_Boolean b1 =
          (yConst >= y0 || yConst >= y1) && (yConst <= y0 || yConst <= y1);
      Standard_Boolean b2 =
          (x0 >= xMin && x0 <= xMax || x1 >= xMin && x1 <= xMax);
      if (b1 && b2) {
...
PositionType Reverse(PositionType t) {
  if (t == OuterPosition) return InnerPosition;
  if (t == InnerPosition) return OuterPosition;
  return t;
}

void GetEdgeInfo(
    const std::vector<MeshLinesAndContoursIntersections::Cross>& lints,
    Standard_Real uBeg, Standard_Real uEnd, PositionType& t1, PositionType& t2,
  ...
  for (; it != lints.end(); ++it) {
    ...
    if (u >= uBeg - eps) {
      if (u >= uBeg - eps && u <= uBeg + eps) t1 = BoundaryPosition;
      break;
    }
    tt1 = Reverse(tt1);
  }

  if (it == lints.end()) return;

  PositionType tt2 = tt1;
  for (; it != lints.end(); ++it) {
    Standard_Real u = it->u;
    if (u >= uEnd - eps) {
      if (u >= uEnd - eps && u <= uEnd + eps) t2 = BoundaryPosition;
      break;
    }
    tt2 = Reverse(tt2);
    if (!(*seg1)) {
  ...
  if (t1 != BoundaryPosition) t1 = tt1;
  if (t2 != BoundaryPosition) t2 = tt2;
}

void GetCellLineIndices(const MeshLinesAndContoursIntersections& mlacInts,
                        const QuadTree& qTree, const QuadTree::Cell& cell,
                        Standard_Integer& i1Line, Standard_Integer& j1Line,
                        Standard_Integer& i2Line, Standard_Integer& j2Line) {
...
PositionType ClassificateCell(const QuadTree& qTree, const QuadTree::Cell& cell,
  ...
  */
  ...
  avType = a1Type;
  assert(a1Type == a2Type);
  bvType = b1Type;
  assert(b1Type == b2Type);
  cvType = c1Type;
  assert(c1Type == c2Type);
  dvType = d1Type;
  assert(d1Type == d2Type);
  ...
  for (const auto* seg : segments) {
    ...
    if (intSegment1 == seg || intSegment1 == seg->prev) continue;
  ...
  Standard_Boolean hasInnerContour =
      ContourLieInnerCell(fContours, xMin, yMin, xMax, yMax);
...
void CalcLineCoefficients(const gp_Pnt2d& p1, const gp_Pnt2d& p2,
...
Standard_Boolean ContourLieInnerCell(const FaceContours& fContours,
  ...
  for (const auto& bb : fContours.myBoundBoxes) {
    ...
    if (xMinCell <= xMinBox && xMaxCell >= xMaxBox && yMinCell <= yMinBox &&
        yMaxCell <= yMaxBox)
...
MeshLinesAndContoursIntersections::MeshLinesAndContoursIntersections(
    const QuadTree& qTree, const FaceContours& fContours,
...
AuxClassificateData2d::AuxClassificateData2d(const TopoDS_Face& F,
                                             Standard_Real tol)
  ...
  MeshLinesAndContoursIntersections mlacInts(*qTree, fContours,
                                             maxLevelsNumber);
  ...
  for (Standard_Integer level = 0; level < maxLevelsNumber; ++level) {
    ...
    for (QuadTree::Cell cell : qTree->TerminalCellsOfLevel(level)) {
      ...
      PositionType type =
          ClassificateCell(*qTree, cell, fContours, mlacInts, &node);
