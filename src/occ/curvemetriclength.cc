...
Standard_Real PlaneFunction(const gp_Pnt& P, Standard_Real A, Standard_Real B,
                            Standard_Real C, Standard_Real D) {
  return A * P.X() + B * P.Y() + C * P.Z() + D;
}

Standard_Real LineFunction(const gp_Pnt2d& P, Standard_Real A, Standard_Real B,
                           Standard_Real C) {
  return A * P.X() + B * P.Y() + C;
}

//! Параметр пересечения кривой с плоскостью.
Standard_Real GetIntersection(const Handle(Geom_Curve) & C,
                              Standard_Real uFirst, Standard_Real uSecond,
                              const gp_Pln& plane) {
  ...
  if (f0 < 0.0 && f1 < 0.0 || f0 > 0.0 && f1 > 0.0) return HUGE_VAL;

  Standard_Real u0 = uFirst, u1 = uSecond;
  if (f0 > 0.0) std::swap(u0, u1);
...
//! Параметр пересечения кривой с плоскостью.
Standard_Real GetIntersection(const Handle(Geom2d_Curve) & C,
                              Standard_Real uFirst, Standard_Real uSecond,
                              const gp_Lin2d& line) {
  Standard_Real a, b, c;
...
Standard_Real MaximumGapParameter(const Handle(Geom_Curve) & C,
                                  Standard_Real u0, Standard_Real u1) {
...
//! Наибольшее расстояние между частью кривой и упирающейся в нее хордой.
Standard_Real MaximumGapParameter(const Handle(Geom2d_Curve) & C,
                                  Standard_Real u0, Standard_Real u1) {
...
Standard_Real pow2(double v) { return v * v; }

Standard_Real pow3(double v) { return v * v * v; }
}  // namespace

CurveMetricLength::CurveMetricLength(const Handle(Geom_Curve) & C,
  ...
  while (Standard_True) {
    auto itNext = it;
    ++itNext;
    if (itNext == myPolyline.end()) break;
...
Standard_Real CurveMetricLength::Length() const { return myL; }

Standard_Real CurveMetricLength::ParameterByAbscissa(
    Standard_Real uStart, Standard_Real abscissa) const {
  assert(uStart >= myU0 && uStart <= myU1);

  gp_Pnt ptStart = myCurve->Value(uStart);
  auto it = myPolyline.begin();
  while (it != myPolyline.end() && *it <= uStart) ++it;
  ...
  while (allL < abscissa) {
    ++it;
    if (it == myPolyline.end()) return HUGE_VAL;
    u0 = u1, m0 = m1;
    u1 = (*it);
...
Standard_Real CurveMetricLength::CalcCurveMetricLength(Standard_Real uA,
                                                       Standard_Real uB,
                                                       Standard_Real hA,
                                                       Standard_Real hB) const {
...
Standard_Real CurveMetricLength::MetricOnCurvePoint(Standard_Real u) const {
  ...
  Standard_Real alpha =
      std::sqrt(6 * myRelAccuracy) / std::pow(1 + dro * dro, 0.25);
  gp_Pnt p;
  gp_Vec v1, v2;
...
Curve2dMetricLength::Curve2dMetricLength(const Handle(Geom2d_Curve) & C,
...
Standard_Real Curve2dMetricLength::Length() const { return myL; }
...
Standard_Real Curve2dMetricLength::CalcCurveMetricLength(
    Standard_Real uA, Standard_Real uB, Standard_Real hA,
    Standard_Real hB) const {
  // Pascal Jean Frey, Paul-Louis George.
  // Mesh Generation Application to Finite Elements / p.465
