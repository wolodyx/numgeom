...
class NUMGEOM_EXPORT QuadTree {
 ...
 public:
  ...
  static QuadTree::Ptr Create(const Bnd_Box2d&, Standard_Integer in = 1,
 ...
 public:
  ...
  void GetBox(Standard_Real& xMin, Standard_Real& yMin, Standard_Real& xMax,
              Standard_Real& yMax) const;

  void CellsNumberOnLevel(Standard_Integer level, Standard_Integer& nbByI,
  ...
  void GetCellBox(const Cell& cell, Standard_Real& xMin, Standard_Real& yMin,
  ...
  void GetMinMaxDistances(const gp_Pnt2d& q, const Cell& cell,
 ...
 private:
  QuadTree(const Bnd_Box2d&, Standard_Integer in, Standard_Integer jn);
...
struct QuadTree::Cell {
  ...
  Cell(Standard_Integer lvl, Standard_Integer idx) : level(lvl), index(idx) {}
  ...
  Standard_Boolean operator<(const Cell& other) const {
    if (level < other.level) return Standard_True;
    if (level > other.level) return Standard_False;
...
struct QuadTree::Node {
  ...
  Node(Standard_Integer lvl, Standard_Integer idx) : level(lvl), index(idx) {}

  Standard_Boolean operator!() const { return level < 0; }

  Standard_Boolean operator==(const Node& other) const {
    if (other.level == this->level) return other.index == this->index;
