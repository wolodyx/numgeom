...
struct OcTree::Internal {
  ...
  Cell GetChildCell(const Cell& parentCell, Standard_Integer childIndex) const;

  void GetCellsOfNextLevel(const Cell& parentCell,
                           std::array<Standard_Integer, 8>& cells) const;

  //! Размерность подложки уровня `level`.
  Ijk LevelSize(Standard_Integer level) const;

  Standard_Integer GetNbCellsOnLevel(Standard_Integer level) const;
  ...
  gp_Pnt PointCoords(const Node& node) const;

  Node Normalize(const Node& node) const;

  void Nodes(const Cell& cell, std::array<Node, 8>& nodes) const;
};
...
Standard_Boolean OcTree::Internal::IsTerminal(const Cell& cell) const {
  ...
  if (cell.level == levels - 1) return Standard_True;
...
Standard_Integer OcTree::Internal::GetOrder(const Cell& cell) const {
  ...
  static const Standard_Integer s_cellOrder[2][2][2] = {{{0, 1}, {3, 2}},
                                                        {{4, 5}, {7, 6}}};

  return s_cellOrder[b3 ? 0 : 1][b2 ? 0 : 1][b1 ? 0 : 1];
}

OcTree::Cell OcTree::Internal::GetChildCell(const Cell& parentCell,
                                            Standard_Integer childIndex) const {
  ...
  switch (childIndex) {
    case 0:
      return Cell(parentCell.level + 1, Ijk::Index(Ijk(c.i, c.j, c.k), n));
    case 1:
      return Cell(parentCell.level + 1, Ijk::Index(Ijk(c.i + 1, c.j, c.k), n));
    case 2:
      return Cell(parentCell.level + 1,
                  Ijk::Index(Ijk(c.i + 1, c.j + 1, c.k), n));
    case 3:
      return Cell(parentCell.level + 1, Ijk::Index(Ijk(c.i, c.j + 1, c.k), n));
    case 4:
      return Cell(parentCell.level + 1, Ijk::Index(Ijk(c.i, c.j, c.k + 1), n));
    case 5:
      return Cell(parentCell.level + 1,
                  Ijk::Index(Ijk(c.i + 1, c.j, c.k + 1), n));
    case 6:
      return Cell(parentCell.level + 1,
                  Ijk::Index(Ijk(c.i + 1, c.j + 1, c.k + 1), n));
    case 7:
      return Cell(parentCell.level + 1,
                  Ijk::Index(Ijk(c.i, c.j + 1, c.k + 1), n));
  }
  return Cell();
}

void OcTree::Internal::GetCellsOfNextLevel(
    const Cell& parentCell, std::array<Standard_Integer, 8>& cells) const {
  Ijk n = this->LevelSize(parentCell.level);
  Ijk c = Ijk::Coords(parentCell.index, n);
...
Ijk OcTree::Internal::LevelSize(Standard_Integer level) const {
  ...
  return Ijk(nbByI * r, nbByJ * r, nbByK * r);
}
...
OcTree::Cell OcTree::Internal::GetParentCell(const Cell& cell) const {
  if (cell.level == 0) return Cell();
  Ijk c = this->GetCellCoords(cell);
  Ijk n = this->LevelSize(cell.level);
  return Cell(cell.level - 1, Ijk::Index(c / 2, n / 2));
}

Standard_Boolean OcTree::Internal::IsExists(const Cell& cell) const {
  Standard_Integer nbLevels = this->Levels();
  if (cell.level >= nbLevels) return Standard_False;
  ...
  if (cell.index >= nc) return Standard_False;

  if (cell.level == 0) return Standard_True;

  Cell parentCell = this->GetParentCell(cell);
  const auto& ntCells = nonTerminalCells[parentCell.level];
  return std::binary_search(ntCells.begin(), ntCells.end(), parentCell.index);
}

OcTree::Cell OcTree::Internal::GetFirstTerminalCell(
    const Cell& startCell) const {
  ...
  if (beNonTerminal) {
    ...
    do {
      ...
      if (level >= nonTerminalCells.size()) break;
    ...
    while (true) {
      ...
      if (beNonTerminalParentCell) break;
...
OcTree::Cell OcTree::Internal::Next(const Cell& startCell) const {
  ...
  while (true) {
    if (cell.level == 0) {
      ...
      if (nextCellIndex == nbByI * nbByJ * nbByK) return Cell{0, nextCellIndex};
...
gp_Pnt OcTree::Internal::PointCoords(const Node& node) const {
  Ijk n = this->LevelSize(node.level);
...
OcTree::Node OcTree::Internal::Normalize(const Node& node) const {
  ...
  if (node.level != 0) {
    while (c.i != 0 && c.j != 0 && c.k != 0 && c.i % 2 == 0 && c.j % 2 == 0 &&
           c.k % 2 == 0 && n.i % 2 == 0 && n.j % 2 == 0 && n.k % 2 == 0) {
      c /= 2;
      n /= 2;
      ++levelDown;
    }
...
void OcTree::Internal::Nodes(const Cell& cell,
                             std::array<Node, 8>& nodes) const {
  Ijk c = this->GetCellCoords(cell);
  Ijk n = this->LevelSize(cell.level);
  n += 1;
  Standard_Integer nij = n.i * n.j;
  nodes[0] =
      this->Normalize(Node(cell.level, Ijk::Index(Ijk(c.i, c.j, c.k), n)));
  nodes[1] =
      this->Normalize(Node(cell.level, Ijk::Index(Ijk(c.i + 1, c.j, c.k), n)));
  nodes[2] = this->Normalize(
      Node(cell.level, Ijk::Index(Ijk(c.i + 1, c.j + 1, c.k), n)));
  nodes[3] =
      this->Normalize(Node(cell.level, Ijk::Index(Ijk(c.i, c.j + 1, c.k), n)));
  nodes[4] =
      this->Normalize(Node(cell.level, Ijk::Index(Ijk(c.i, c.j, c.k + 1), n)));
  nodes[5] = this->Normalize(
      Node(cell.level, Ijk::Index(Ijk(c.i + 1, c.j, c.k + 1), n)));
  nodes[6] = this->Normalize(
      Node(cell.level, Ijk::Index(Ijk(c.i + 1, c.j + 1, c.k + 1), n)));
  nodes[7] = this->Normalize(
      Node(cell.level, Ijk::Index(Ijk(c.i, c.j + 1, c.k + 1), n)));
}

OcTree::OcTree(const Bnd_Box& boundBox, Standard_Integer in,
               Standard_Integer jn, Standard_Integer kn) {
  pimpl = new Internal();
...
OcTree::~OcTree() { delete pimpl; }

Standard_Integer OcTree::Levels() const { return pimpl->Levels(); }

void OcTree::GetBox(Standard_Real& xMin, Standard_Real& yMin,
                    Standard_Real& zMin, Standard_Real& xMax,
...
namespace {
;
Standard_Integer CellsCount(const OcTree::Internal* oTreeInternal,
...
class IteratorImpl_OcTreeCells : public IteratorImpl<OcTree::Cell> {
 ...
 public:
  IteratorImpl_OcTreeCells(const OcTree::Internal* oTreeInternal,
  ...
  OcTree::Cell current() const override { return myCurrentCell; }
  ...
  IteratorImpl<OcTree::Cell>* last() const override {
    Standard_Integer N = myOTreeInternal->nbByI * myOTreeInternal->nbByJ *
                         myOTreeInternal->nbByK;
    return new IteratorImpl_OcTreeCells(myOTreeInternal, OcTree::Cell{0, N});
  }

  bool end() const override { return !myCurrentCell; }
...
class IteratorImpl_OcTreeCellsOfLevel : public IteratorImpl<OcTree::Cell> {
 ...
 public:
  IteratorImpl_OcTreeCellsOfLevel(const OcTree::Internal* oTreeInternal,
    ...
    if (!myOTreeInternal->IsTerminal(cell)) GoToNextTerminalCell();
  }
  ...
  void GoToNextTerminalCell() {
    ...
    for (++myCurrentIndex; myCurrentIndex < myNbCellsOnLevel;
         ++myCurrentIndex) {
      if (itNtCells == itNtCellsEnd) break;

      if (myCurrentIndex < (*itNtCells)) break;
  ...
  void advance() override { this->GoToNextTerminalCell(); }
  ...
  IteratorImpl<OcTree::Cell>* clone() const override {
    return new IteratorImpl_OcTreeCellsOfLevel(myOTreeInternal, myLevel,
                                               myCurrentIndex);
  }
  ...
  bool end() const override { return myCurrentIndex == myNbCellsOnLevel; }
...
class IndexIterator {
 ...
 public:
  ...
  IndexIterator(const OcTree::Internal* oTreeInternal, const OcTree::Cell& cell,
    ...
    if (this->isBlockedCell()) this->operator++();
  }
  ...
  IndexIterator& operator++() {
    ...
    for (++myCurrentCellCoords; !myCurrentCellCoords.isEnd();
         ++myCurrentCellCoords) {
      if (this->isBlockedCell()) continue;
      break;
    }
    return *this;
  }
  ...
  Standard_Boolean More() const { return !myCurrentCellCoords.isEnd(); }

  Standard_Boolean Empty() const { return myUpLevel == -1; }

  void SetEnd() { myCurrentCellCoords.setEnd(); }

  Standard_Boolean operator==(const IndexIterator& other) const {
    return myISize == other.myISize && myJSize == other.myJSize &&
           myCellCoords == other.myCellCoords &&
           myCurrentCellCoords == other.myCurrentCellCoords &&
           myUpLevel == other.myUpLevel && myQMultiple == other.myQMultiple &&
...
class IteratorImpl_ConnectedCells : public IteratorImpl<OcTree::Cell> {
 ...
 public:
  IteratorImpl_ConnectedCells(
      const OcTree::Internal* oTreeInternal, const OcTree::Cell& cell,
    ...
    if (myIndexIterator.Empty())
      myIndexIterator =
          IndexIterator(oTreeInternal, cell, oTreeInternal->Levels() - 1);
  ...
  IteratorImpl<OcTree::Cell>* clone() const override {
    return new IteratorImpl_ConnectedCells(myOTreeInternal, myBaseCell,
                                           myIndexIterator);
  }
  ...
  bool end() const override { return !myIndexIterator.More(); }
...
void OcTree::GetCellBox(const Cell& cell, Standard_Real& xMin,
                        Standard_Real& yMin, Standard_Real& zMin,
                        Standard_Real& xMax, Standard_Real& yMax,
...
Standard_Boolean OcTree::Dump(const std::filesystem::path& fileName) const {
  std::ofstream file(fileName);
  if (!file.is_open()) return Standard_False;
  ...
  for (Standard_Integer i = 0; i < nbCells; ++i)
    file << "8 " << 8 * i + 0 << ' ' << 8 * i + 1 << ' ' << 8 * i + 2 << ' '
         << 8 * i + 3 << ' ' << 8 * i + 4 << ' ' << 8 * i + 5 << ' '
         << 8 * i + 6 << ' ' << 8 * i + 7 << std::endl;
  file << std::endl;

  file << "CELL_TYPES " << nbCells << std::endl;
  for (size_t i = 0; i < nbCells; ++i) file << "12 ";  //< VTK_HEXAHEDRON
  file << std::endl;
...
void OcTree::Split(const Cell& cell) {
  ...
  if (it == ntCells.end() || *it != cell.index) ntCells.insert(it, cell.index);
}
...
OcTree::Cell OcTree::GetCell(const gp_Pnt& Q) const {
  ...
  if (xQ < xMin || xQ > xMax || yQ < yMin || yQ > yMax || zQ < zMin ||
      zQ > zMax)
    return Cell();
  ...
  for (Standard_Integer level = 0;; ++level) {
    ...
    if (pimpl->IsTerminal(cell)) return cell;
  }
  return Cell();
}

size_t OcTree::GetAttr(const Cell& cell) const {
  if (cell.level >= pimpl->attrs.size()) return 0;
...
void OcTree::SetAttr(const Cell& cell, size_t attr) const {
  ...
  while (cell.level >= pimpl->attrs.size()) pimpl->attrs.push_back({});
...
Standard_Boolean OcTree::Equals(OcTree::CPtr other) const {
  ...
  pimpl->boundBox.Get(bbCoords[0], bbCoords[1], bbCoords[2], bbCoords[3],
                      bbCoords[4], bbCoords[5]);

  std::array<Standard_Real, 6> bbCoords2;
  other->pimpl->boundBox.Get(bbCoords2[0], bbCoords2[1], bbCoords2[2],
                             bbCoords2[3], bbCoords2[4], bbCoords2[5]);

  for (int i = 0; i < 6; ++i) {
    Standard_Real dt = std::abs(bbCoords[i] - bbCoords2[i]);
    if (dt > 1.e-7) return Standard_False;
  }
...
void OcTree::Serialize(std::ostream& stream) const {
  ...
  if (!pimpl->attrs.empty()) data["Attrs"] = pimpl->attrs;
...
OcTree::Ptr OcTree::Deserialize(std::istream& stream) {
  ...
  Bnd_Box boundBox;
  {
    ...
    if (!jBoundBox.is_array() || jBoundBox.size() != 6) return OcTree::Ptr();
    Standard_Real xMin, yMin, zMin, xMax, yMax, zMax;
  ...
  Standard_Integer in, jn, kn;
  {
    ...
    if (!jZeroLevelCellsNumbers.is_array() ||
        jZeroLevelCellsNumbers.size() != 3)
      return OcTree::Ptr();
...
OcTree::Ptr OcTree::Create(const Bnd_Box& boundBox, Standard_Integer in,
                           Standard_Integer jn, Standard_Integer kn) {
  if (in < 1 || jn < 1 || kn < 1) return OcTree::Ptr();
...
namespace {
;
OcTree::Cell GetParentCell(OcTree::Internal* qTreeIntenal,
                           const OcTree::Cell& cell,
                           Standard_Integer parentLevel) {
  assert(cell.level >= parentLevel);
  if (cell.level == parentLevel) return cell;
...
OcTree::CellConnection OcTree::GetConnectionType(const Cell& aCell,
                                                 const Cell& bCell) const {
...
void OcTree::GetMinMaxDistances(const gp_Pnt& q, const Cell& cell,
  ...
  Standard_Real dxMin = std::min(std::abs(xQ - xMin), std::abs(xQ - xMax));
  Standard_Real dxMax = std::max(std::abs(xQ - xMin), std::abs(xQ - xMax));
  Standard_Real dyMin = std::min(std::abs(yQ - yMin), std::abs(yQ - yMax));
  Standard_Real dyMax = std::max(std::abs(yQ - yMin), std::abs(yQ - yMax));
  Standard_Real dzMin = std::min(std::abs(zQ - zMin), std::abs(zQ - zMax));
  Standard_Real dzMax = std::max(std::abs(zQ - zMin), std::abs(zQ - zMax));
...
struct CellWithDistances {
  ...
  CellWithDistances(OcTree::CPtr tree, const OcTree::Cell& c, const gp_Pnt& q)
      : cell(c) {
...
void PushNonVisitedCellsToQueue(OcTree::CPtr tree,
                                const OcTree::Cell& startCell, const gp_Pnt& Q,
...
void SearchNearestCells(
    ...
    const std::function<Standard_Boolean(OcTree::CPtr, const OcTree::Cell&)>&
        isTaggedCell,
    Standard_Real maximumDistance, std::list<OcTree::Cell>& nearestCells) {
  nearestCells.clear();
  ...
  while (!queue.empty()) {
    ...
    if (cwd.mn > maximumDistance) continue;

    result.push_back(cwd);
  }

  std::sort(result.begin(), result.end());
  for (const auto& r : result) nearestCells.push_back(r.cell);
}
