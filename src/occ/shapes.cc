...
TriMesh::Ptr MakeSphere(const TriMesh::NodeType& center, double radius,
  ...
  for (size_t iStack = 0; iStack < nStacks - 1; ++iStack) {
    ...
    for (size_t iSlice = 0; iSlice < nSlices; ++iSlice) {
      ...
      TriMesh::NodeType p(radius * cos_u * cos_v, radius * cos_u * sin_v,
                          radius * sin_u);
...
glm::mat4 CreateTransformFromLineToZ(const glm::vec3& pointA,
...
TriMesh::Ptr MakeCylinder(const TriMesh::NodeType& s,
...
TriMesh::Ptr MakeCone(const TriMesh::NodeType& s, const TriMesh::NodeType& t,
  ...
  size_t iNode = 0;
  mesh->GetNode(iNode++) =
      TriMesh::NodeType(0, 0, 0);  //< We'll update it later.
  for (size_t iSlice = 0; iSlice < nSlices; ++iSlice) {
    double t = iSlice * 2 * M_PI / nSlices;
    double x = radius * std::cos(t);
    double y = radius * std::sin(t);
  ...
  for (size_t iSlice = 0; iSlice < nSlices; ++iSlice)
    mesh->GetCell(iSlice) =
        TriMesh::Cell(0, iSlice + 1, 1 + (iSlice + 1) % nSlices);
