...
Dumper::Dumper() { pimpl = new Internal; }

Dumper::~Dumper() { delete pimpl; }
...
Dumper& Dumper::AddPolyline(const std::vector<gp_Pnt2d>& polyline) {
  ...
  for (const auto& p : polyline) pl.push_back(gp_Pnt(p.X(), p.Y(), 0.0));
  return *this;
}
...
Standard_Boolean Dumper::Save(const std::filesystem::path& fileName) const {
  ...
  for (const auto& pl : pimpl->polylines) nbPoints += pl.size();
  ...
  for (const auto& pl : pimpl->polylines) {
    for (const auto& pt : pl) file << pt.X() << ' ' << pt.Y() << ' ' << pt.Z();
    file << std::endl;
  }

  size_t nbVerts = pimpl->points.size();
  size_t nbLines = 0;
  for (const auto& pl : pimpl->polylines) nbLines += (pl.size() - 1);

  file << "CELLS " << nbVerts + nbLines << ' ' << 2 * nbVerts + 3 * nbLines
       << std::endl;
  for (size_t i = 0; i < nbVerts; ++i) file << " 1 " << i;
  file << std::endl;
  ...
  for (size_t i = 0; i < nbVerts; ++i) file << "1 ";
  file << std::endl;
  for (size_t i = 0; i < nbLines; ++i) file << "3 ";
  file << std::endl;
