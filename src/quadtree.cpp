...
struct QuadTree::Internal
{
    ...
    //! Ќомера нетерминальных €чеек на уровне.
    ...
    //! –азмерность подложки уровн€ `level`.
    void LevelSize(
...
) const
{
    ...
    // ќт пор€дкового номера €чейки переходим к координатам.
    Standard_Integer i = parentCell.index % ni;
    Standard_Integer j = parentCell.index / ni;
...
) const
{
    ...
    // ячейка существует тогда, когда в подложке есть €чейка с таким номером и
    // €чейка входит в нулевой уровень или если нет, то ее родитель в списке
    // нетерминальных.
...
) const
{
    ...
    if(beNonTerminal)
    {
        // ≈сли €чейка попадает в список нетерминальных,
        // то поднимаемс€ выше пока она оттуда не исключитс€.
        do
        {
            i *= 2, j *= 2, ni *= 2;
            ++level;
    ...
    {
        // ≈сли €чейки нет в списках нетерминальных €чеек, то спускаемс€
        // пока родительска€ €чейка не окажетс€ в списках нетерминальных.
        while(true)
        {
            if(level == 0)
                break;
...
class IteratorImpl_QuadTreeCells : public IteratorImpl<QuadTree::Cell>
{
    ...
    QuadTree::Cell myCurrentCell; //!< “екуща€ терминальна€ €чейка.
public:
...
//!\brief »тератор дл€ обхода €чеек сетки уровн€ выше вокруг €чейки уровн€ ниже.
class IndexIterator
{
    //!  оординаты базовой €чейки.
    Standard_Integer myICell, myJCell;
    //! ѕризнак, что нижн€€, права€, верхн€€ и лева€ сторона упираютс€ в границу.
    Standard_Boolean myBlockedSize[4];
    //!  оординаты текущей €чейки итератора.
    Standard_Integer myCurrentI,myCurrentJ;
    //! ”ровень сетки, €чейки которой обходит итератор.
...
public:
    ...
    IndexIterator& operator++()
    {
        ...
        if(beOnSide[0])
        {
            ...
            {
                ++myCurrentI; //< ѕеремещаемс€ вправо.
                // ”перлись в правую сторону?
...
QuadTree::Cell QuadTree::GetCell(const gp_Pnt2d& Q) const
{
    ...
    // “очка расположена за пределами габаритной коробки дерева.
...
{
    // ≈сли €чейки расположены на разных уровн€х,
