...
static void handle_xdg_toplevel_configure(void *data, xdg_toplevel *toplevel,
                                          int32_t width, int32_t height,
                                          wl_array *states) {}

static void handle_xdg_toplevel_close(void *data, xdg_toplevel *toplevel) {}
...
static void handle_xdg_wm_base_ping(void *data, xdg_wm_base *shell,
                                    uint32_t serial) {
...
static void handle_wl_keyboard_key(void *data, wl_keyboard *wl_keyboard,
                                   uint32_t serial, uint32_t time, uint32_t key,
                                   uint32_t state) {
  if (key == KEY_ESC && state == WL_KEYBOARD_KEY_STATE_PRESSED) std::exit(0);
}
...
static void registry_handle_global(void *data, wl_registry *registry,
  ...
  if (strcmp(interface, "wl_compositor") == 0) {
    wl->compositor = (wl_compositor *)wl_registry_bind(
        registry, name, &wl_compositor_interface, 1);
  } else if (strcmp(interface, "xdg_wm_base") == 0) {
    wl->shell = (xdg_wm_base *)wl_registry_bind(registry, name,
...
bool init_wayland_connection(wl_connection *wl) {
  ...
  if (!wl->shell)
    throw std::runtime_error(
        "Compositor is missing xdg_wm_base protocol support");
...
VkSurfaceKHR create_wayland_surface(VkInstance instance,
  ...
  CreateWaylandSurface(instance, &waylandSurfaceCreateInfoKHR, nullptr,
                       &surface);
