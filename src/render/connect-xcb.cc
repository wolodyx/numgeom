...
bool init_xcb_connection(uint16_t width, uint16_t height, xcb_connection* xcb) {
  ...
  xcb_screen_t* screen = nullptr;
  {
    ...
    for (int i = 0; i < screenNumber; ++i) xcb_screen_next(&it);
    screen = it.data;
  }
  ...
  value_list[1] = XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_RESIZE_REDIRECT |
                  XCB_EVENT_MASK_KEY_PRESS | XCB_EVENT_MASK_KEY_RELEASE |
                  XCB_EVENT_MASK_POINTER_MOTION | XCB_EVENT_MASK_BUTTON_PRESS |
                  XCB_EVENT_MASK_BUTTON_RELEASE;
  xcb_void_cookie_t cookieWindow = xcb_create_window_checked(
      connection, XCB_COPY_FROM_PARENT, win, screen->root, 0, 0, width, height,
  ...
  xcb_intern_atom_reply_t* reply =
      xcb_intern_atom_reply(connection, cookie1, 0);
  ...
  xcb_change_property(connection, XCB_PROP_MODE_REPLACE, win, (*reply).atom, 4,
                      32, 1, &(*atomReply).atom);
  free(reply);

  // Устанавливаем заголовок окна.
  const char* title = "numgeom";
  xcb_change_property(connection, XCB_PROP_MODE_REPLACE, win, XCB_ATOM_WM_NAME,
...
VkSurfaceKHR create_xcb_surface(VkInstance instance,
  ...
  VkResult r;
  r = vkCreateXcbSurfaceKHR(instance, &surfaceCreateInfoKHR, nullptr, &surface);
  assert(r == VK_SUCCESS);
  return surface;
}
  ...
  if (event) {
    switch (event->response_type & ~0x80) {
      ...
      case XCB_KEY_PRESS: {  //< Нажата клавиша.
        ...
        if (key_event->detail == 0x9 || key_event->detail == 0x18) quit = true;
        break;
      }
      case XCB_RESIZE_REQUEST:
        break;
      default:
        // std::cout << "opcode = " << (event->response_type & ~0x80) <<
        // std::endl;
        break;
    }
