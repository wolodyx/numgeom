...
static int find_host_coherent_memory(struct vkcube *vc, unsigned allowed) {
  for (unsigned i = 0;
       (1u << i) <= allowed && i <= vc->memory_properties.memoryTypeCount;
       ++i) {
    if ((allowed & (1u << i)) &&
        (vc->memory_properties.memoryTypes[i].propertyFlags &
...
/**
Создаем:
* размещение набора дескрипторов;
* размещение конвейера;
* шейдерные модули;
* графический конвейер;
* буфер с выделенной памятью и инициализированными данными;
* пул дескрипторов;
* наборы дескрипторы, связанные с буфером;

*/
static void init_cube(struct vkcube *vc) {
  VkResult r;
  ...
  VkDescriptorSetLayoutCreateInfo descriptorSetLayoutCreateInfo{
      ...
      .bindingCount = 1,
      .pBindings = bindings};
  vkCreateDescriptorSetLayout(vc->device, &descriptorSetLayoutCreateInfo,
                              nullptr, &set_layout);
  ...
  vkCreatePipelineLayout(vc->device, &pipelineLayoutCreateInfo, nullptr,
                         &vc->pipeline_layout);
  ...
  vkCreateShaderModule(vc->device, &shaderModuleCreateInfo1, nullptr,
                       &vs_module);
  ...
  vkCreateShaderModule(vc->device, &shaderModuleCreateInfo2, nullptr,
                       &fs_module);
  ...
  VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo{
      ...
      .basePipelineIndex = 0};
  vkCreateGraphicsPipelines(vc->device, VK_NULL_HANDLE, 1,
  ...
  vkCreateBuffer(vc->device, &bufferCreateInfo, nullptr, &vc->buffer);
  ...
  if (memory_type < 0)
    BOOST_LOG_TRIVIAL(fatal) << "find_host_coherent_memory failed";
  ...
  vkAllocateMemory(vc->device, &memoryAllocateInfo, nullptr, &vc->mem);

  r = vkMapMemory(vc->device, vc->mem, 0, mem_size, 0, &vc->map);
  if (r != VK_SUCCESS) BOOST_LOG_TRIVIAL(fatal) << "vkMapMemory failed";
  memcpy(static_cast<char *>(vc->map) + vc->vertex_offset, vVertices,
         sizeof(vVertices));
  memcpy(static_cast<char *>(vc->map) + vc->colors_offset, vColors,
         sizeof(vColors));
  memcpy(static_cast<char *>(vc->map) + vc->normals_offset, vNormals,
         sizeof(vNormals));
  ...
  vkAllocateDescriptorSets(vc->device, &descriptorSetAllocateInfo,
                           &vc->descriptor_set);
  ...
  vkUpdateDescriptorSets(vc->device, 1, writeDescriptorSet, 0, nullptr);
}

static void render_cube(struct vkcube *vc, struct vkcube_buffer *b,
                        bool wait_semaphore) {
  struct ubo ubo;

  auto now = std::chrono::system_clock::now();
  uint64_t t =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - vc->start_tv)
          .count();

  ubo.mv = glm::mat4x4(1.0);
  ubo.mv = glm::translate(ubo.mv, glm::vec3(0.0f, 0.0f, -8.0f));
  ubo.mv = glm::rotate(ubo.mv, -glm::radians(45.0f + (0.25f * t)),
                       glm::vec3(1.0f, 0.0f, 0.0f));
  ubo.mv = glm::rotate(ubo.mv, -glm::radians(45.0f - (0.5f * t)),
                       glm::vec3(0.0f, 1.0f, 0.0f));
  ubo.mv = glm::rotate(ubo.mv, -glm::radians(10.0f + (0.15f * t)),
                       glm::vec3(0.0f, 0.0f, 1.0f));

  float aspect = (float)vc->height / (float)vc->width;
  glm::mat4x4 projection =
      glm::frustum(-2.8f, +2.8f, -2.8f * aspect, +2.8f * aspect, 6.0f, 10.0f);
  ...
  vkBeginCommandBuffer(b->cmd_buffer, &commandBufferBeginInfo);
  ...
  vkCmdBeginRenderPass(b->cmd_buffer, &renderPassBeginInfo,
                       VK_SUBPASS_CONTENTS_INLINE);

  VkBuffer buffers[] = {vc->buffer, vc->buffer, vc->buffer};
  VkDeviceSize offsets[] = {vc->vertex_offset, vc->colors_offset,
                            vc->normals_offset};
  vkCmdBindVertexBuffers(b->cmd_buffer, 0, 3, buffers, offsets);

  vkCmdBindPipeline(b->cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    vc->pipeline);

  vkCmdBindDescriptorSets(b->cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          vc->pipeline_layout, 0, 1, &vc->descriptor_set, 0,
                          nullptr);
...
struct model cube_model = {.init = init_cube, .render = render_cube};
