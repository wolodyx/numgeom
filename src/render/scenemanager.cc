...
SceneManager::SceneManager() { myPimpl = new Internal; }

SceneManager::~SceneManager() {
  for (auto e : myPimpl->entities) delete e;
  delete myPimpl;
}
...
bool SceneManager::add(SceneEntity* e) {
  ...
  if (it == myPimpl->entities.end()) myPimpl->entities.push_back(e);
  return true;
}
...
bool SceneManager::remove(SceneEntity* e) {
  ...
  auto it = std::find(myPimpl->entities.begin(), myPimpl->entities.end(), e);
  if (it == myPimpl->entities.end()) return false;
