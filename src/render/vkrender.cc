...
void init_vulkan(vkcube* vc,
  ...
  VkResult res = vkCreateInstance(&instanceCreateInfo, nullptr, &vc->instance);
  if (res != VK_SUCCESS)
  ...
  vkGetPhysicalDeviceMemoryProperties(vc->physical_device,
                                      &vc->memory_properties);

  // Выбираем семейство очередей.
  vkGetPhysicalDeviceQueueFamilyProperties(vc->physical_device, &count,
                                           nullptr);
  assert(count > 0);
  ...
  VkDeviceCreateInfo deviceCreateInfo{
      ...
      .enabledExtensionCount =
          static_cast<uint32_t>(deviceEnabledExtensionNames.size()),
      .ppEnabledExtensionNames = deviceEnabledExtensionNames.data(),
  };
  vkCreateDevice(vc->physical_device, &deviceCreateInfo, nullptr, &vc->device);
...
static void init_vk_objects(vkcube* vc) {
  ...
  vkCreateRenderPass(vc->device, &renderPassCreateInfo, nullptr,
                     &vc->render_pass);
  ...
  vkCreateCommandPool(vc->device, &commandPoolCreateInfo, nullptr,
                      &vc->cmd_pool);

  VkSemaphoreCreateInfo semaphoreCreateInfo{
      .sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO,
  };
  vkCreateSemaphore(vc->device, &semaphoreCreateInfo, nullptr, &vc->semaphore);
}
...
static void init_buffer(vkcube* vc, vkcube_buffer* b) {
  ...
  vkCreateImageView(vc->device, &imageViewCreateInfo, nullptr, &b->view);
  ...
  VkFramebufferCreateInfo framebufferCreateInfo{
      ...
      .height = vc->height,
      .layers = 1};
  vkCreateFramebuffer(vc->device, &framebufferCreateInfo, nullptr,
                      &b->framebuffer);

  VkFenceCreateInfo fenceCreateInfo{
      .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO,
      .flags = VK_FENCE_CREATE_SIGNALED_BIT};
  vkCreateFence(vc->device, &fenceCreateInfo, nullptr, &b->fence);
  ...
  vkAllocateCommandBuffers(vc->device, &commandBufferAllocateInfo,
                           &b->cmd_buffer);
}

/* Swapchain-based code - shared between XCB and Wayland */

static VkFormat choose_surface_format(vkcube* vc) {
...
static void create_swapchain(vkcube* vc) {
  ...
  if (minImageCount < surface_caps.minImageCount) {
    if (surface_caps.minImageCount > MAX_NUM_IMAGES) {
      BOOST_LOG_TRIVIAL(fatal) << std::format(
          "surface_caps.minImageCount is too large (is: {}, max: {})",
  ...
  vkCreateSwapchainKHR(vc->device, &swapchainCreateInfoKHR, nullptr,
                       &vc->swap_chain);

  vkGetSwapchainImagesKHR(vc->device, vc->swap_chain, &vc->image_count,
                          nullptr);
...
static void fini_buffer(vkcube* vc, vkcube_buffer* b) {
...
static void recreate_swapchain(vkcube* vc) {
