...
Standard_Real BruteForceCalcOfNearestDistance(
    ...
    const std::function<Standard_Boolean(
        QuadTree::CPtr, const QuadTree::Cell&)>& isTaggedCell) {
  ...
  for (QuadTree::Cell cell : qTree->TerminalCells()) {
    if (!isTaggedCell(qTree, cell)) continue;
...
TEST(QuadTree, SearchNearestCells) {
  ...
  for (QuadTree::Cell cell : qTree->TerminalCells()) {
    ...
    SearchNearestCells(qTree, q, IsTaggedCell,
                       std::numeric_limits<double>::max(), nearestCells);
    ...
    Standard_Real nearestDistance =
        BruteForceCalcOfNearestDistance(qTree, q, IsTaggedCell);
    Standard_Real dMin, dMax;
